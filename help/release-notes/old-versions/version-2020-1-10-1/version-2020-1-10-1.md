...
O **Substance Designer 2020.1** inclui um novo editor de atalhos, um plug-in MaterialX e várias novas melhorias no gerenciamento de cores com as funcionalidades de Adobe Color Engine e novos padeiros.
...
Refinamos alguns dos nossos padeiros com duas mudanças principais:

* **Amostragem aprimorada**\
  A Oclusão ambiente, o Thickness e o padeiro de curvatura agora usam um novo método de amostragem que melhora muito a qualidade dos resultados do cozimento. Produz um ruído mais atraente e resultados mais estáveis, sem a necessidade de aumentar a contagem de amostras. A imagem abaixo mostra o método de amostra anterior na parte superior e o novo método na parte inferior com da esquerda para a direita 4, 8 e 16 amostras (todas as renderizações foram feitas com suavização de serrilhado definida para subamostragem 4x4).

  ![](../../../assets/bakers-sampling-comparison.jpg){width="600px"}
* **Nova configuração de normalização**\
  Os padeiros Thickness e Heightmap têm um novo parâmetro de normalização que permite preparar valores brutos para a textura em vez de valores fixados no intervalo 0-1. Isso é útil com um mapa de height para conhecer as distâncias brutas entre uma malha de baixo-poli e alto-poli, por exemplo. O valor do divisor corresponderá exatamente à intensidade do deslocamento a ser configurada na malha de baixo polígono para corresponder à silhueta de malha de alto polígono. Para interpretar corretamente o mapa de height, também expusemos um novo parâmetro “Valor zero escalar” para definir o valor de pixel correspondente a 0. Além disso, o fator manual permite corrigir possíveis problemas de emenda nas malhas UDIM causados por uma normalização por UV-Tile.
...
* **Sincronização de parâmetros entre renderizadores**\
  Os parâmetros de sombreador agora estão sincronizados entre OpenGl e Iray, o que facilita muito o salto entre eles.
* **Exibir visualização de entrada de textura e adicionar parâmetro de desabilitação**\
  Os parâmetros de entrada do sombreador agora exibem uma visualização de sua entrada: pode ser um parâmetro de cor uniforme, um nó de gráfico ou um bitmap. Também é possível desmarcar um parâmetro para desativá-lo temporariamente.

  ![](../../../assets/3dview-map-preview.png)

  ![](../../../assets/3dview-enable-setting.gif)
* **Visualização do mapa de ambiente nas configurações**\
  Ao ir para **Exibição 3D > Ambiente > Editar**, agora há uma visualização do mapa de ambiente usado para iluminar a cena.

  ![](../../../assets/3dview-env-settings.jpg)
* **Novo sombreador não iluminado**\
  Um novo sombreador chamado “unlit” foi incluído por padrão. Ele não reage a nenhuma iluminação e, em vez disso, pode ser usado para exibir uma única textura na janela de visualização sobre uma malha. Isso é útil para examinar texturas assadas, por exemplo.
...
* **Novo filtro FXAA**\
  O novo filtro implementa o algoritmo de suavização aproximada de borda rápida (FXAA), que é um método de suavização de borda. Pode ser usado em texturas normais para suavizar linhas ou bordas irregulares.\

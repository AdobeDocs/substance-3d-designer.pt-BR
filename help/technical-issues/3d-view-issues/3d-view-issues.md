...
A *distância* do deslocamento pela qual os vértices são movidos é calculada da seguinte forma: `Distance = Height scale * Height map`. Como o mapa de altura *não foi alterado* no gráfico, isso deixa a **escala de Height**.

O valor padrão da escala de Height é **1.0**, que pode resultar em um efeito de deslocamento *não perceptível* dependendo da malha exibida no Visualização 3D e do mapa de altura aplicado a ele.
...
Nas configurações padrão, o Visualização 3D retornará automaticamente para o renderizador OpenGL se a opção &#39;Renderizador padrão&#39; estiver definida como &#39;Padrão (renderizador predefinido)&#39; nas [Configurações do projeto](../../interface/preferences-window/project-settings/project-settings.md).
...
> 
> No entanto, a maioria das GPUs AMD e Intel também não é compatível e produzirá uma renderização em preto sem mensagem. Consulte o item “Visualização 3D é totalmente preto” acima para obter orientações sobre essas GPUs.

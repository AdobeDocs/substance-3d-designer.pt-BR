...
O [Visualização 3D](../../../interface/3d-view/3d-view.md) renderiza a superfície dos modelos usando um programa chamado *sombreador*. O sombreador define o material
aplicadas ao modelo usando uma lista de propriedades que afetam vários aspectos da aparência do modelo.

O menu **Materiais** do Visualização 3D permite verificar qual sombreador é usado para cada um dos materiais da cena.
...
| Cor | Precisão decimal3 (RGB) | 0.8, 0.8, 0.8 | Cor do reflexo da base difusa e metálica. |
...
| Cor | Precisão decimal3 (RGB) | 1.0, 1.0, 1.0 | Cor do reflexo do specular. (Controla a tonalidade da borda física para metais,<br/>e uma tonalidade geral não física para dielétricos) |
...
| Cor | Precisão decimal3 (RGB) | 1.0, 1.0, 1.0 | Controla a cor da base transparente devido à lei de Beer volumétrica<br/>absorção sob a superfície. |
...
| Dispersão | Precisão decimal3 (RGB) | 0.0, 0.0, 0.0 | Controla a cor da luz dispersa volumetricamente dentro da base transparente. |
...
| Cor | Precisão decimal3 (RGB) | 0.8, 0.8, 0.8 | A cor de reflexão observada do meio de dispersão subsuperficial. |
| Raio | Float | 1.0 | Escala de comprimento do caminho livre médio da dispersão subsuperficial. |
| Escala do raio | Precisão decimal3 (RGB) | 1.0, 0.5, 0.25 | multiplicador de RGB para subsurface_radius, fornecendo dispersão por canal<br/>caminhos sem média. |
...
| Parâmetro | Tipo | Padrão | Descrição |
|------------|--------------|---------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
| Peso | Float | 0.0 | O peso de presença de uma camada refletora de revestimento transparente em cima do material.<br/>Use para materiais como tinta de carro ou camada oleosa. |
| Cor | Precisão decimal3 (RGB) | 1.0, 1.0, 1.0 | A cor da transparência da camada de verniz transparente, devido à absorção no revestimento. |
...
| Cor | Precisão decimal3 (RGB) | 1.0, 1.0, 1.0 | A cor da camada difusa. |
...
| Cor | Precisão decimal3 (RGB) | 1.0, 0.0, 0.0 | A cor da luz emitida. |
...
| Parâmetro | Tipo | Padrão | Descrição |
|-----------|-------|---------|-------------------------------------------------------------------------------------------------------|
| Peso | Float | 0.0 | Peso da cobertura da película fina.<br/>Use para materiais como tinta de carro multitons ou bolhas de sabão. |
...
| Normal | Precisão decimal3 (RGB) | 0.5, 0.5, 1.0 | Insira a normal geométrica para a superfície. |
| Tangente | Precisão decimal3 (RGB) | 1.0, 0.5, 0.0 | Insira a tangente geométrica. |
| Normal do Revestimento | Precisão decimal3 (RGB) | 0.5, 0.5, 1.0 | Entrada normal para a camada de revestimento. |
| Tangente do revestimento | Precisão decimal3 (RGB) | 1.0, 0.5, 0.0 | Insira a tangente geométrica para a camada de revestimento. |
...
| Oclusão Ambiente | Float | 1.0 | Oclusão de ambiente mapa para escurecer áreas ocultadas.<br/>Branco (1.0) significa totalmente iluminado, preto (0.0) significa totalmente ocultado. |
...
entre renderizadores que incluem USD e/ou Hydra.

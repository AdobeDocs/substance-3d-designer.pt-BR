---
helpx_url: "https://helpx.adobe.com/substance-3d-designer/substance-compositing-graphs/nodes-reference-for-substance-compositing-graphs/node-library/filters/normal-map/height-to-normal-world-units.html"
breadcrumb-title: ''
description: Use o nó Height para unidades do mundo normal para converter mapas de height em mapas normais usando o dimensionamento de unidades do mundo para detalhes precisos.
helpx_creative_field: ""
helpx_description: Designer > Substance compositing graphs > Nodes reference for Substance compositing graphs > Node library > Filters > Normal Map > Height to Normal World Units
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Height para unidades do mundo normal
user-guide-description: ''
user-guide-title: ''
source-git-commit: 6c55ac0f1f6da5bc5683a34a4eca174f978eac64
workflow-type: tm+mt
source-wordcount: '135'
ht-degree: 1%

---


# Height para unidades do mundo normal

<table>
<tr style="border: 0;">
<td style="border: 0;" valign="top">

![](../../../../../../assets/normal-hq.png){width="128px"}

## Height para unidades do mundo normal

**Entrada:** *Filtros/Mapa Normal*

**Simples**

</td>
<td style="border: 0;" valign="top">

## Descrição

Um nó de conversão avançado Height-para-normal que faz uso de unidades do mundo real durante a conversão.

Útil para quando você conhece as dimensões do mapa de altura de origem e deseja executar a conversão mais precisa, como ao trabalhar com material digitalizado.

## Parâmetros

* **Tamanho da Superfície (cm)**: *0.0 - 1000.0* Dimension do Heightmap de entrada.
* **Profundidade de Height (cm)**: *0.0 - 100.0* profundidade máxima de detalhes de Heightmap.
* **Formato Normal**: *OpenGL, DirectX*\
  Alterna entre diferentes formatos de Mapas Normais (inverte o canal verde).
* **Amostragem**: *Padrão, Sobel* Alterna entre dois modos de amostragem determinando a precisão.

## Imagens de exemplo

|  |
| --- |
| Não há imagens anexadas a esta página. |

</td>
</tr>
</table>
